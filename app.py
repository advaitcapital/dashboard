...
DASHBOARD_URL = "https://dashboardgit-bw5ekcpbs9wcpd9uz2t8mu.streamlit.app"
...
# RBI repo rate — changes only 6-8 times/year at MPC meetings.
# FRED series INTDSRINM193N is discontinued (OECD-MEI). Seeded manually.
# Last MPC: Aug 5, 2026 — held at 5.25%. Next MPC: Oct 5-7, 2026.
LATEST_RBI_REPO = 5.25
PREV_RBI_REPO = 5.50      # was 5.50% before Dec 2025 cut
...
def wgb_yields(country):
    """Scrape government bond yields from worldgovernmentbonds.com.
    Free, no API key, simple HTML, works on Streamlit Cloud.
    Returns {'1Y': float, '2Y': float, '5Y': float, '10Y': float, ...} or {}."""
    slugs = {"india": "india", "uk": "united-kingdom", "us": "united-states",
             "germany": "germany", "japan": "japan"}
    slug = slugs.get(country)
    if not slug:
        return {}
    url = f"http://www.worldgovernmentbonds.com/country/{slug}/"
    try:
        r = requests.get(url, headers={"User-Agent": "Mozilla/5.0 (market-dashboard)"}, timeout=15)
        if r.status_code != 200:
            return {}
        import re as _re
        out = {}
        # The page has a table with tenor labels (1Y, 2Y, 5Y, 10Y etc) and yields
        # Pattern: look for rows with tenor and yield percentage
        # The yield table has patterns like: >1 Year</... >5.92%< or >6.79%<
        rows = _re.findall(
            r'(?:>(\d+)\s*(?:Year|Month|year|month)s?<.*?)'
            r'(?:>(\d+\.\d+)\s*%<)',
            r.text, _re.S | _re.I
        )
        for tenor_num, yield_val in rows:
            try:
                t = int(tenor_num)
                v = float(yield_val)
                if t == 1:
                ...
                pass
        # Alternative pattern: some pages use "1Y" directly
        if not out:
            for tenor in ("1Y", "2Y", "3Y", "5Y", "7Y", "10Y", "30Y"):
                m = _re.search(
                    rf'(?:{tenor}|{tenor[0]}\s*(?:Year|year))'
                    rf'.*?(\d+\.\d+)\s*%',
                    r.text, _re.S | _re.I
                )
                if m:
                    out[tenor] = float(m.group(1))
        return out
    except Exception:
        return {}


@st.cache_data(ttl=300)  # 5 min cache — CNBC updates frequently
def cnbc_yields(country="india"):
    """Fetch government bond yields from CNBC's free JSON API.
    Works on Streamlit Cloud (plain HTTP, no API key, no browser).
    Returns {'1Y': float, '2Y': float, '10Y': float} or {}."""
    symbols = {
        "india": {"1Y": "IN1Y-IN", "2Y": "IN2Y-IN", "10Y": "IN10Y-IN"},
        "uk":    {"1Y": "GB1Y-GB", "2Y": "GB2Y-GB", "10Y": "GB10Y-GB"},
    }
    syms = symbols.get(country, {})
    if not syms:
        return {}
    all_syms = "|".join(syms.values())
    url = (f"https://quote.cnbc.com/quote-html-webservice/restQuote/symbolType/symbol"
           f"?symbols={all_syms}&requestMethod=itv&noencoding=1"
           f"&partnerId=2&fund=1&exthrs=1&output=json")
    try:
        ...
        if isinstance(quotes, dict):
            quotes = [quotes]
        out = {}
        sym_to_tenor = {v: k for k, v in syms.items()}
        for q in quotes:
            sym = q.get("symbol", "")
            tenor = sym_to_tenor.get(sym)
            if tenor:
                try:
                    out[tenor] = round(float(q.get("last", 0)), 2)
                except (ValueError, TypeError):
                    pass
        return out
    except Exception:
...
def market_summary(eq, fx, cm, gold):
    """Build a short, data-driven '10 AM summary' from the live numbers.
    Returns a list of plain-text bullet lines."""
    lines = []
    ...
    if is_monday:
        ...
        prev_day = (today - dt.timedelta(days=1)).strftime("%A")
        vs_label = f"vs {prev_day}"
    ...
    _spx, smv = emv("^BSESN")
    if nmv is not None:
        seg = f"Nifty 50 at {npx:,.0f} ({nmv:+.2f}% {vs_label})"
        if smv is not None:
            seg += f", Sensex {smv:+.2f}%"
    ...
    if u and u.get("d1") is not None:
        lines.append(f"Rupee: USD/INR {u['price']:.2f} ({u['d1']:+.2f}% {vs_label}).")

    # Gold (live Indian rate + % change)
    g = (gold or {}).get("24k")
    gmv = cm.get("GC=F", {}).get("d1") if cm.get("GC=F") else None
    if g:
        seg = f"Gold 24K Rs {g * 10:,.0f}/10g"
        if gmv is not None:
            seg += f" ({gmv:+.2f}% {vs_label})"
        lines.append(seg + ".")

    # Biggest move: the largest absolute index move
    if moves:
...
def build_excel(report, meta, summary=None):
    ...
    r += 1
    if summary:
        c = ws.cell(r, 1, "10 AM Summary"); c.font = Font(bold=True, size=12); r += 1
        for s in summary:
            cell = ws.cell(r, 1, "• " + s)
...
/* ============================================================
   REMOVE ALL STREAMLIT BRANDING — every icon, badge, toolbar
   ============================================================ */

/* Top toolbar: Fork, GitHub, Deploy, 3-dot menu */
...
/* Bottom-right badges: Streamlit logo, paper boat, profile popup */
footer,
footer:after,
.reportview-container .main footer,
.viewerBadge_container__r5tak,
.viewerBadge_link__qRIXo,
...
/* Nuclear: hide ALL iframes (Streamlit injects badges via iframes) */
.stApp iframe {
    display:none !important;
    visibility:hidden !important;
}

/* Nuclear: hide anything fixed to bottom-right corner */
...
/* Keep ONLY the sidebar hamburger toggle */
[data-testid="collapsedControl"] {
    visibility:visible !important;
    display:flex !important;
    position:relative !important;
    top:auto !important; height:auto !important; width:auto !important;
    pointer-events:auto !important;
    opacity:1 !important;
}
.mchg-lbl{font-size:.6em;color:var(--muted);font-weight:500;letter-spacing:.02em}

/* Cover the bottom-right badges with a dark overlay matching the background */
.stApp::after {
    content: "";
...
with st.sidebar:
    ...
    with st.expander("India macro (CPI / WPI / GDP — update on release)"):
        ...
        rbi_in = st.text_input("RBI Repo Rate (%)",
                               value=str(manual.get("rbi_repo", "")),
                               placeholder=str(LATEST_RBI_REPO))
        ...
        if st.button("Save India macro"):
            save_manual({"india_cpi": c_in.strip(), "india_gdp": g_in.strip(),
                         "india_wpi": w_in.strip(), "rbi_repo": rbi_in.strip(),
...
try:
    ...
    with _lcol2:
        ...
        st.markdown('<a href="https://advaitcapital.co.in" target="_blank" '
                    'style="color:#5b8cff;font-size:12px;text-decoration:none">'
                    '🌐 advaitcapital.co.in</a>', unsafe_allow_html=True)
except Exception:
    st.title("Daily Market Dashboard")
st.caption(f"🕐 {dt.datetime.now(IST):%a %d %b %Y  %H:%M:%S} IST  ·  "
           f"Last updated {dt.datetime.now(IST):%H:%M:%S} IST")
...
if _summary:
    ...
    st.markdown(
        '<div class="summary-box"><div class="summary-h">10 AM Summary</div>'
...
_ind_short = india_short_yields()  # India manual/FRED fallback
_cnbc_india = cnbc_yields("india")  # CNBC live
_cnbc_uk = cnbc_yields("uk")        # CNBC live
_wgb_india = wgb_yields("india")    # WorldGovernmentBonds — primary
_wgb_uk = wgb_yields("uk")          # WorldGovernmentBonds — primary
...
def _yield_cell(country, tenor):
    ...
    # 1. WorldGovernmentBonds.com (primary — free, live, all tenors)
    if country == "india":
        v = _wgb_india.get(tenor)
    ...
    # 2. CNBC live API (fallback for India and UK)
    if country == "india":
        v = _cnbc_india.get(tenor)
        if v is not None and v > 0:
            return f"{v:.2f}%"
    if country == "uk":
        v = _cnbc_uk.get(tenor)
        if v is not None and v > 0:
            return f"{v:.2f}%"

    # 3. Dedicated cloud-friendly API fetchers (fallback)
    if country == "japan":
        v = _jgb.get(tenor)
        if v is not None:
            return f"{v:.2f}%"
...
st.markdown('<div class="src">India &amp; UK = WorldGovernmentBonds.com (primary), '
            'CNBC (fallback); US = Treasury.gov + FRED; Germany = ECB; '
            'Japan = MOF JGB curve.</div>',
            unsafe_allow_html=True)
...
# INTDSRINM193N is dead. Use seeded RBI repo rate instead.
_ff_now, _ff_chg = policy_rate_changes("FEDFUNDS", FRED_API_KEY)
_rbi_now = LATEST_RBI_REPO
_rbi_chg = {"m1": 0, "m3": 0, "m6": 0,
            "y1": round((LATEST_RBI_REPO - PREV_RBI_REPO) * 100, 0)}
...
_rbi_manual = str(manual.get("rbi_repo", "")).strip()
if _rbi_manual:
    try:
        _rbi_now = float(_rbi_manual)
    except Exception:
        pass

_rbi_cell = f"{_rbi_now:.2f}%"
