...
DASH = "https://dashboardgit-bw5ekcpbs9wcpd9uz2t8mu.streamlit.app"
...
def summary(eq,fx,g,cmd=None):
    ...
    if fp: ls.append(f"USD/INR {fp:.2f}" + (f" ({fc:+.2f}% {vs})" if fc else ""))
    if g:
        gold_chg = ""
        if cmd:
            gc = cmd.get("GC=F")
            if gc and gc.get("d1") is not None:
                gold_chg = f" ({gc['d1']:+.2f}% {vs})"
        ls.append(f"Gold 24K Rs {g*10:,.0f}/10g{gold_chg}")
    return ls
...
def send_email(pdf,summ):
    ...
    html = f"""<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
      ...
      <div style="background:#f4f6f9;border-radius:8px;padding:16px 20px;margin:16px 0">
        <h3 style="margin:0 0 8px;font-size:14px;color:#1E2761">10 AM Summary</h3>
...
def send_whatsapp(pdf_bytes, summary_lines):
    wa_token = os.environ.get("WA_TOKEN", "")
    phone_id = os.environ.get("WA_PHONE_ID", "")
    template = os.environ.get("WA_TEMPLATE", "daily_market_reports")
    recipients = [x.strip() for x in os.environ.get("WA_RECIPIENTS", "").split(",") if x.strip()]

    if not all([wa_token, phone_id, recipients]):
        print("\nWhatsApp: SKIPPED (missing WA_TOKEN/WA_PHONE_ID/WA_RECIPIENTS)")
        return

    api = f"https://graph.facebook.com/v21.0/{phone_id}/messages"
    media_api = f"https://graph.facebook.com/v21.0/{phone_id}/media"
    headers = {"Authorization": f"Bearer {wa_token}"}

    # Step 1: Upload the PDF to Meta's servers
    print("\nWhatsApp: Uploading PDF...")
    ...
    try:
        ...
        if upload_resp.status_code != 200:
            print(f"  PDF upload failed: {upload_resp.status_code} {upload_resp.text}")
            media_id = None
        else:
    ...
    # Step 2: Build the summary text for {{1}}
    bullets = "\n".join(f"• {s}" for s in summary_lines)

    # Step 3: Send to each recipient
    print(f"WhatsApp: Sending to {len(recipients)} recipients...")
    for num in recipients:
        to = num if num.startswith("91") else f"91{num}"
        payload = {
        ...
        if media_id:
            components.append({
                ...
                "parameters": [{
                    "type": "document",
                    "document": {"id": media_id, "filename": fname}
                }]
            })

        # Body component (summary + link)
        components.append({
            "type": "body",
            "parameters": [
        ...
        try:
            resp = requests.post(api, headers={**headers, "Content-Type": "application/json"},
                                 json=payload, timeout=20)
            if resp.status_code == 200:
                msg_id = resp.json().get("messages", [{}])[0].get("id", "ok")
                print(f"  ✓ Sent to +{to}: {msg_id}")
            else:
                print(f"  ✗ Failed +{to}: {resp.status_code} {resp.text}")
        except Exception as e:
            print(f"  ✗ Error +{to}: {e}")
...
if __name__ == "__main__":
    ...
    send_email(pdf, su)
    try:
        send_whatsapp(pdf, su)
    except Exception as e:
        print(f"\nWhatsApp FAILED: {e}")
        print("Email was sent successfully. WhatsApp error does not affect email.")
