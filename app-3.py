...
_cnbc_india = cnbc_yields("india")  # CNBC live — primary source for India
_cnbc_uk = cnbc_yields("uk")        # CNBC live — primary source for UK
...
def _yield_cell(country, tenor):
    ...
    # 1. CNBC live API (primary source for India and UK — free, real-time)
    if country == "india":
    ...
    # 2. Dedicated cloud-friendly API fetchers (fallback)
    if country == "japan":
        v = _jgb.get(tenor)
        if v is not None:
            return f"{v:.2f}%"
...
st.markdown('<div class="src">India &amp; UK = CNBC live bond quotes (primary); '
            'fallback: BoE CSV API for UK, FRED/manual for India. '
            'US = Treasury.gov + FRED; Germany = ECB euro-area AAA '
            'curve; Japan = MOF JGB curve.</div>',
            unsafe_allow_html=True)
