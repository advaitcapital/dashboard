...
def send_whatsapp(pdf_bytes, summary_lines):
    ...
    if not all([sid, token, wa_from, recipients]):
        print("\nWhatsApp: SKIPPED (missing TWILIO_SID/TOKEN/WA_FROM/WA_RECIPIENTS)")
        return

    from twilio.rest import Client
    client = Client(sid, token)

    # Build the message text (same summary as email)
    today = dt.date.today().strftime("%d %b %Y")
    bullets = "\n".join(f"• {s}" for s in summary_lines)
    msg_text = (f"📊 *Daily Market Dashboard*\n"
                f"_{today} • 9:30 AM IST_\n\n"
                f"*Biggest move*\n{bullets}\n\n"
                f"🔗 {DASH}")

    print(f"\nWhatsApp: Sending to {len(recipients)} recipients...")

    for num in recipients:
        to = f"whatsapp:+{num}"
        try:
            # Send text summary
            m = client.messages.create(body=msg_text, from_=wa_from, to=to)
            print(f"  ✓ Text to {num}: {m.sid}")

            # Send PDF as media
            # Upload PDF to a temporary public URL via Twilio's media
            import base64
            fname = f"market-dashboard-{dt.date.today().isoformat()}.pdf"

            # For sandbox, we need to host the PDF somewhere accessible.
            # Twilio can send media from a public URL. We'll use a data URI workaround:
            # Actually, Twilio sandbox supports sending media via ContentSid or URL.
            # The simplest approach: save PDF locally and use Twilio's built-in media upload.

            # Note: Twilio sandbox has limited media support. Send the text summary
            # which contains the dashboard link where they can download the PDF.
            # For full media support, upgrade to a paid WhatsApp number.
